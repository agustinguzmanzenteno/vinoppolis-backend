...
exports.eliminarCategoria = (req, res) => {
  db.query('SELECT * FROM imagen WHERE entidadId = ? AND tipoEntidad = "categoria"', [req.params.id], (err, result) => {
    ...
    const eliminarCategoriaImagen = () => {
      db.query('DELETE FROM imagen WHERE entidadId = ? AND tipoEntidad = "categoria"', [req.params.id], (errImgDel) => {
        if (errImgDel) return res.status(500).send(errImgDel);

        db.query('DELETE FROM categoria WHERE id = ?', [req.params.id], (err2) => {
          if (err2) return res.status(500).send(err2);
          res.json({ message: 'Categoría e imagen eliminadas correctamente' });
        });
      });
    };
